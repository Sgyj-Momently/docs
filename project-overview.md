...
이 프로젝트는 여러 장의 사진과 동영상을 입력받아 구조화된 정보로 변환하고, 목적에 맞게 그룹화한 뒤, 대표 사진 선택과 글 생성까지 이어지는 에이전트 기반 파이프라인을 만드는 것을 목표로 한다.
...
- 특징:
  - Ollama 기반 비전 모델 분석
  - 동영상은 `ffmpeg`로 여러 대표 프레임을 추출한 뒤 요약 병합
  - 텍스트 모델 기반 bundle 요약
